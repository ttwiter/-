// --- Sinyal/Analisis Trading Bagian Asli ---
const allowedPairs = [
...
function getLiveIcon() {
  return `<span class="icon-live" title="Realtime Analytic">
    <svg width="21" height="21" viewBox="0 0 21 21" fill="none"><rect x="2" y="6" width="3" height="9" rx="1.5" fill="#32b7ff"/><rect x="8" y="2" width="3" height="17" rx="1.5" fill="#32b7ff"/><rect x="14" y="8" width="3" height="5" rx="1.5" fill="#32b7ff"/></svg>
  </span>`;
}
function getSignalIcon() {
  return `<span style="margin-left:6px;vertical-align:-2px;">
    <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M6 16a6 6 0 0 1 8.5 0" stroke="#FFA500" stroke-width="2.1" stroke-linecap="round"/><path d="M9.5 16.5a2 2 0 0 1 3 0" stroke="#FFA500" stroke-width="2.1" stroke-linecap="round"/><circle cx="11" cy="8.5" r="2.5" fill="#FFA500" stroke="#FFA500" stroke-width="2.1"/></svg>
  </span>`;
}
function renderSignals() {
  const container = document.getElementById('container');
  container.innerHTML = '';
  // Render setiap provider
  ...
  // --- Kolom Iklan/Pengumuman (tambahan) ---
  renderIklan();
}
...
// --- Kolom Iklan/Pengumuman (tambahan) ---
async function renderIklan() {
  const res = await fetch('/iklan.json');
  const iklan = await res.json();
  const container = document.getElementById('iklan-container');
  container.innerHTML = iklan.length ? iklan.map(ik =>
    `<div class="iklan-item" style="border:1px solid #ffb71b;background:#fffbe7;color:#222;padding:8px 12px;margin:6px 0 10px 0;border-radius:8px;">
      <span>📢 ${ik.text}</span>
    </div>`
  ).join('') : "";
}

window.onload = renderSignals;
